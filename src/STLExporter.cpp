...
void STLExporter::exportBinary(std::ofstream&& ofs) {
    ...
    std::string header = std::format("UNITS=cm. STL file generated by Molumes software at {:%H:%M} {:%d/%m/%y} :)", getCurrentTime(date), date);
...
std::vector<STLExporter::Plane> STLExporter::zipNormalsAndVertices(const std::vector<glm::vec4> &vertices) {
    const auto normals = calculateNormals(normalizeRange(vertices));
